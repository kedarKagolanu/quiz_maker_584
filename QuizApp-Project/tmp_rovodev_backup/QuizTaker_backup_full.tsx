...
export const QuizTaker: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadQuiz = async () => {
      ...
      // Handle multi-quiz mode
      if (multiQuizMode === 'true' && quizSources) {
        try {
          ...
          console.error('Failed to build multi-quiz:', error);
      ...
      // Apply randomization after question limiting
      const qs = quizWithCustomSettings.randomize ? finalQuestions.sort(() => Math.random() - 0.5) : finalQuestions;
  ...
  const getQuestionStatusColor = (index: number) => {
    if (markedForReview[index]) return 'bg-yellow-500 hover:bg-yellow-400 text-black font-bold border-2 border-yellow-300';
    if (answers[index] !== -1) return 'bg-green-500 hover:bg-green-400 text-white font-bold border-2 border-green-300';
    if (questionStatus[index] === 'seen') return 'bg-orange-500 hover:bg-orange-400 text-white font-bold border-2 border-orange-300';
    return 'bg-gray-600 hover:bg-gray-500 text-white font-bold border-2 border-gray-400';
  };
      ...
      <div className={`grid gap-2 ${navPosition === 'bottom' ? 'grid-cols-12' : 'grid-cols-4'}`}>
        {questions.map((_, idx) => (
          <button
            ...
            className={`w-10 h-10 rounded text-sm font-bold transition-colors ${getQuestionStatusColor(idx)} ${currentIndex === idx ? 'ring-2 ring-terminal-accent ring-offset-2' : ''} ${hasPerQuestionTimer && idx < currentIndex ? 'opacity-50 cursor-not-allowed' : ''}`}
